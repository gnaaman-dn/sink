...
impl ResponseAsyncReader {
    ...
    fn poll_chunk(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<std::io::Result<Bytes>> {
...
impl AsyncBufRead for ResponseAsyncReader {
    fn poll_fill_buf(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<std::io::Result<&[u8]>> {
